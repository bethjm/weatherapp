...
let randomSpringTempRange = () => {
    ...
    if (SpringTemprange <= .15 ){
        showOnScreen.push('A')
    } else if (SpringTemprange >= .15 && SpringTemprange <= .60){
        showOnScreen.push('B')
    } else if (SpringTemprange >= .60 && SpringTemprange <= .85){
        showOnScreen.push('C')
    } else if (SpringTemprange >= .85 && SpringTemprange <= .90){
        showOnScreen.push('D')
    } else if (SpringTemprange >= .90 && SpringTemprange <= .95){
        showOnScreen.push('E')
...
let randomSpringHumidityRange = () => {
    ...
    }  else if (SpringHumidityrange <= .2 ){
        showOnScreen.push('low')
    } else if (SpringHumidityrange > .2 && SpringHumidityrange <= .6){
        showOnScreen.push('medium')
    } else if (SpringHumidityrange > .6 && SpringHumidityrange <= .8){
        showOnScreen.push('high')
    } else if (SpringHumidityrange > .8 && SpringHumidityrange <= 1){
        showOnScreen.push('very high')
    } 
}
...
let springPercip = () => {
    ...
    if (showOnScreen[1] > 90){
        ...
        if (highPercipRange <.5){
            let range = percipitationArray['springAndSpringThunderAndLightningDanger'][Math.floor(Math.random()*percipitationArray['springAndSpringThunderAndLightningDanger'].length)]
            showOnScreen.push(range)
        } else if (highPercipRange > .5 && highPercipRange < .7){
            let range = percipitationArray['SpringThunderAndLightning'][Math.floor(Math.random()*percipitationArray['SpringThunderAndLightning'].length)]
            showOnScreen.push(range)
...
let springStateOfSky = () => {
    ...
    } else if (SpringSkyRange < .3){
        showOnScreen.push('cloudy')
    } else if (SpringSkyRange > .3 && SpringSkyRange < .9){
        showOnScreen.push('sunny')
    }  else if (showOnScreen[4] != 't&l' && SpringSkyRange > .9 && SpringSkyRange < 1){
...
let springStateOfWind = () => {
    ...
    console.log(printOnScreenSpring())
