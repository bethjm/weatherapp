...
let randomFallTempRange = () => {
    ...
    if (FallTemprange <= .15 ){
        showOnScreen.push('A')
    } else if (FallTemprange >= .15 && FallTemprange <= .60){
        showOnScreen.push('B')
    } else if (FallTemprange >= .60 && FallTemprange <= .85){
        showOnScreen.push('C')
    } else if (FallTemprange >= .85 && FallTemprange <= .90){
        showOnScreen.push('D')
    } else if (FallTemprange >= .90 && FallTemprange <= .95){
        showOnScreen.push('E')
    } else if (FallTemprange >= .95 && FallTemprange <= 1){
        showOnScreen.push('F')
    }
}
...
let randomFallHumidityRange = () => {
    ...
    }  else if (FallHumidityrange <= .2 ){
        showOnScreen.push('low')
    } else if (FallHumidityrange > .2 && FallHumidityrange <= .6){
        showOnScreen.push('medium')
    } else if (FallHumidityrange > .6 && FallHumidityrange <= .8){
        showOnScreen.push('high')
    } else if (FallHumidityrange > .8 && FallHumidityrange <= 1){
        showOnScreen.push('very high')
    } 
}
...
let fallPercip = () => {
    ...
    if (showOnScreen[1] > 90){
        ...
        if (highPercipRange <.5){
            let range = percipitationArray['springAndFallThunderAndLightningDanger'][Math.floor(Math.random()*percipitationArray['springAndFallThunderAndLightningDanger'].length)]
            showOnScreen.push(range)
        } else if (highPercipRange > .5 && highPercipRange < .7){
            let range = percipitationArray['FallThunderAndLightning'][Math.floor(Math.random()*percipitationArray['FallThunderAndLightning'].length)]
            showOnScreen.push(range)
...
let fallStateOfSky = () => {
    ...
    } else if (FallSkyRange < .3){
        showOnScreen.push('cloudy')
    } else if (FallSkyRange > .3 && FallSkyRange < .9){
        showOnScreen.push('sunny')
    }  else if (showOnScreen[4] != 't&l' && FallSkyRange > .9 && FallSkyRange < 1){
